...
public:
    ...
    void laserCloudInfoHandler(const liorf::cloud_infoConstPtr &msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            saveKeyFramesAndFactor(timeLaserInfoCur);
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void saveKeyFramesAndFactor(double timeLaserInfoCur)
    {
        if (saveFrame() == false)
            return;

        timestamps.push_back(timeLaserInfoCur);
    ...
    void retrieveTrajectory()
    {
        ...
        for (const auto &key_value : isamCurrentEstimate)
        {
            ...
            const gtsam::Pose3 &pose = isamCurrentEstimate.at<gtsam::Pose3>(key);

            Eigen::Matrix4d matrix = pose.matrix();
            trajectory.push_back(matrix);
        }

        saveTrajectoryToFile(trajectory, "/home/daniel/PhD/RadarSlam/SOTA/Others/lio_sam_ws/src/liorf/estimates/filename_w_loop.txt");
    }

    void saveTrajectoryToFile(const std::vector<Eigen::Matrix4d> &trajectory, const std::string &filename)
    {
        ...
        std::cout << "Saving trajectory of size " << trajectory.size() << " and " << timestamps.size() << " timestamps to " << filename << std::endl;

        for (int t = 0; t < (int)trajectory.size(); t++)
        {
            // Set fixed and precision for the timestamp
            file << std::fixed << std::setprecision(9) << timestamps[t] << " ";

            for (int i = 0; i < 4; ++i)
            {
                for (int j = 0; j < 4; ++j)
                {
                    file << trajectory[t](i, j);
                    if (i != 3 || j != 3)
            ...
            file << std::endl; // Newline after each matrix
        }
